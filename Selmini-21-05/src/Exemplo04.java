import java.util.Scanner;

/* programa em java para ler um número na base ternária.
O seu programa deverá:

1. Validar se o número realmente está na base ternária.
2. Converter o valor para a base decimal.
 */


public class Exemplo04 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        int digito, expoente = 0, auxiliar;
        int ternario, decimal = 0;
        boolean status = true;

        System.out.println("Informe um valor positivo na quase ternária: ");
        ternario = sc.nextInt();
        auxiliar = ternario;


        do{
            digito = ternario % 10;

            if ( digito != 0 && digito != 1 && digito != 2){
                status = false;
                break;
            }
            ternario = ternario / 10;

        } while(ternario > 0);

        if (status) {

            ternario = auxiliar;
            do {
                digito = ternario % 10;
                decimal = (int) (decimal + digito * Math.pow(3, expoente));
                ternario = ternario / 10;
                expoente++;

            }while(ternario > 0);

            System.out.println(auxiliar + " ---> " + decimal);

        } else {
            System.out.println("Inválido!");
        }

    }
}
